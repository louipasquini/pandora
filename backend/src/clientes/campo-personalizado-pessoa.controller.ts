import {
  BadRequestException,
  Body,
  Controller,
  Delete,
  Get,
  HttpCode,
  Param,
  Patch,
  Post,
  Query,
  Req,
} from '@nestjs/common';
import type { Request } from 'express';
import { z, type ZodTypeAny } from 'zod';
import type { AuthContext } from '../auth/guards/jwt-auth.guard';
import { RequerPermissao } from '../auth/rbac/decorators/requer-permissao.decorator';
import { CampoPersonalizadoPessoaService } from './application/campo-personalizado-pessoa.service';
import {
  criarCampoPessoaDefSchema,
  listarCamposPessoaDefSchema,
  patchCampoPessoaDefSchema,
} from './dto/campo-personalizado-pessoa.schema';

function autor(req: Request): string {
  return (req as Request & { auth?: AuthContext }).auth?.sub ?? 'desconhecido';
}
function parse<S extends ZodTypeAny>(schema: S, data: unknown): z.infer<S> {
  const r = schema.safeParse(data);
  if (!r.success) {
    throw new BadRequestException({
      message: 'entrada inválida',
      detalhes: r.error.issues.map((i) => `${i.path.join('.') || '_'}: ${i.message}`),
    });
  }
  return r.data;
}

/**
 * Definições de campo personalizado de `pessoa` (spec 013, CL-02) — espelha
 * `crm/campo-personalizado.controller.ts` (spec 008). `/clientes/admin/
 * campos-personalizados`, permissão `pessoa:gerir_campos_personalizados`.
 * Auditado em `clientes_audit`.
 */
@Controller('clientes/admin/campos-personalizados')
export class CampoPersonalizadoPessoaController {
  constructor(private readonly svc: CampoPersonalizadoPessoaService) {}

  @RequerPermissao('pessoa:gerir_campos_personalizados')
  @Get()
  listar(@Query() q: Record<string, unknown>) {
    const { ativo } = parse(listarCamposPessoaDefSchema, q);
    return this.svc.listar(ativo);
  }

  @RequerPermissao('pessoa:gerir_campos_personalizados')
  @Get(':id')
  detalhe(@Param('id') id: string) {
    return this.svc.detalhe(id);
  }

  @RequerPermissao('pessoa:gerir_campos_personalizados')
  @Post()
  criar(@Body() body: unknown, @Req() req: Request) {
    return this.svc.criar(parse(criarCampoPessoaDefSchema, body), autor(req));
  }

  @RequerPermissao('pessoa:gerir_campos_personalizados')
  @Patch(':id')
  atualizar(@Param('id') id: string, @Body() body: unknown, @Req() req: Request) {
    return this.svc.atualizar(id, parse(patchCampoPessoaDefSchema, body), autor(req));
  }

  @RequerPermissao('pessoa:gerir_campos_personalizados')
  @HttpCode(204)
  @Delete(':id')
  async remover(@Param('id') id: string, @Req() req: Request) {
    await this.svc.remover(id, autor(req));
  }
}
