/**
 * Validação pura de um valor de campo personalizado de `pessoa` contra a sua
 * definição (spec 013, CL-02). Sem I/O. Devolve o valor **canônico** a
 * persistir (sempre string) ou um erro. Valor "vazio" de `TEXTO` sinaliza
 * remoção. Espelha `crm/domain/lead/validar-valor-campo.ts` (spec 008) —
 * duplicada deliberadamente (não importada de `crm`) para manter `clientes`
 * livre de dependência em outro bounded context (Princípio VI).
 */
import type { CampoPersonalizadoTipo } from '@prisma/client';
