...
import type { EntradaAuditoriaCrm } from '../domain';

/**
 * Auditoria do contexto `crm` (spec 007) — forma canônica `RegistroAuditoria` do
 * core (spec 002), `origem = AJUSTE_MANUAL`. **Append-only** (sem UPDATE/DELETE).
 * Simétrico ao `RbacAuditService` (004), `ClientesAuditService` (005) e
 * `IngestaoAuditService` (006).
 *
 * Grava **só quando há delta real** (`jsonIgual(anterior, novo)` → no-op → `false`).
 * **Nunca** recebe valor de segredo: para `integracao`, o chamador passa um
 * marcador (`{ segredo: 'definido' | 'rotacionado' }`) no `valorNovo`.
 */
@Injectable()
export class CrmAdminAuditService {
  constructor(private readonly prisma: PrismaService) {}

  async registrar(e: EntradaAuditoriaCrm): Promise<boolean> {
    ...
    await this.prisma.crmAdminAudit.create({
      data: {
  ...
  async listar(filtro: {
    ...
    const [itens, total] = await this.prisma.$transaction([
      this.prisma.crmAdminAudit.findMany({
        where,
        orderBy: [{ quando: 'desc' }, { id: 'desc' }],
        skip: (filtro.pagina - 1) * filtro.tamanho,
        take: filtro.tamanho,
      }),
      this.prisma.crmAdminAudit.count({ where }),
    ]);
